...
> 写作 / UI 上叫 **Vibe Light**，binary 叫 `VibeLight`（`/Applications/VibeLight.app`），
> CLI 命令叫 `vibelight`（`/usr/local/bin/vibelight`）。
...
| Hook | Command |
|---|---|
| `SessionStart` | `vibelight set waiting` |
| `UserPromptSubmit` | `vibelight set working` |
| `Stop` | `vibelight set waiting` |
| `Notification` | `vibelight set waiting` |
| `SessionEnd` | `vibelight clear` |

CLI 读 stdin JSON 取 `session_id` + `cwd`，原子写入 `~/.vibelight/state.json`。
Mac app 通过 FSEvents 监听这个文件实时响应。

CLI 安装到 `/usr/local/bin/vibelight`（symlink 到 app bundle 内），
首次启动时弹对话框走管理员授权。
